...
Еще обсуждается, приблизительно
...
2.  **Настройте конфиг:**
    Создайте файл `.env` в корне проекта. Вставьте туда текст из **[.env.example](./.env.example)**:
```bash
    HTTP_PORT=8080
	DB_HOST=local
	DB_PORT=5432
...
```
    ...
    * Swagger: `http://localhost:8080/swagger/index.html` // его пока нет
